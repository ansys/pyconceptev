...
class TransmissionLossMapData:
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        component_file_type = cast(
            Literal["TransmissionLossMap"] | Unset, d.pop("component_file_type", UNSET)
        )
        if component_file_type != "TransmissionLossMap" and not isinstance(
            component_file_type, Unset
        ):
            raise ValueError(
                f"component_file_type must match const 'TransmissionLossMap', got '{component_file_type}'"
            )
